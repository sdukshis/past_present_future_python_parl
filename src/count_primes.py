...
def count_primes_multithreading():
    ...
    jobs = [ExecThread(odds[i], LIMIT, CPUS, total_result) for i in range(CPUS)]
    for job in jobs:
        job.start()
    for job in jobs:
        job.join()
...
def count_primes_subinterpreters():
    ...
    odds = [i + 2 for i in range(CPUS)]

    jobs = [SubInthread(odds[i], LIMIT, CPUS, total_result) for i in range(CPUS)]
    for job in jobs:
        job.start()
    for job in jobs:
        job.join()
...
def count_primes_multiprocessing():
    odds = [2*i + 1 for i in range(CPUS)]
    with Pool(CPUS) as p:
       total_result = sum(p.starmap(count_primes_step, [(odds[i], LIMIT, CPUS) for i in range(CPUS)]))
